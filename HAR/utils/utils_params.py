...
def gen_run_folder(path_model_id=''):
    ...
    if not os.path.isdir(path_model_id):
        ...
        run_id = 'human_activity_recognition_' + date_creation
        if path_model_id:
    ...
    run_paths['summary'] = os.path.join(run_paths['path_model_id'], 'summary')
    run_paths['cache_dir'] = os.path.join(run_paths['path_model_id'], 'cache')
